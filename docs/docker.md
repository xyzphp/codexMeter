...
文件中的每行是一条去重后的采样记录。除兼容用的合并文件外，服务还维护 `data/usage-history-weekly.jsonl` 和 `data/usage-history-five-hour.jsonl` 两条独立历史：周额度只按自身使用百分比去重，5 小时额度只按自身使用百分比去重；相同百分比优先保留最近的正常采样，完成去重后各自最多保留最近 48 个不同百分比，因此五小时额度变化不会挤掉周额度历史，反之亦然。记录包含采样时间、5 小时已使用百分比和本周已使用百分比，不包含 OAuth Token、Cookie、Basic Auth 密码或 App API Key。使用 `docker compose down -v` 也不会删除该目录，因为它是宿主机 bind mount。

升级后首次启动时，服务会立即按上述规则压缩已有的两条独立历史文件；如果文件内容发生变化，压缩前快照会保留在对应的 `.bak` 文件中。由于旧版本已经把原始采样截断为 48 条，无法凭空恢复更早且已被覆盖的不同百分比，因此升级后返回点数可能暂时少于 48；后续出现新的百分比时会继续累积，但不会再次加入重复值。
