...
额度历史由后端独立定时采集，后台只在每个整点采集一次，并将记录时间标记为该整点；服务启动后不会立即补采集。前端手动刷新和自动刷新不会额外写入采样记录。手动刷新仍可请求最新上游数据，但历史记录只由后台采集任务写入。上游异常时，后台会按最近一次成功采集的值记录该整点的降级状态，并标记 `stale: true`；连续相同的成功或降级采样会合并，不重复占用变化点。即使前端在 00:00–08:00 暂停自动刷新，后端采集任务仍会继续运行。
