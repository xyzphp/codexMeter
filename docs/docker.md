...
数据库中的 `quota_history` 表完整保留每次定时采样，不按 48 条截断，字段包括采样时间、周额度使用百分比、5 小时额度使用百分比和 `stale` 标记。接口读取完整表后，周额度只按自身 `used_percent` 去重，再取最近最多 48 个不同百分比；5 小时额度按自身 `five_hour_used_percent` 执行同样规则。相同百分比优先保留最近的正常采样，因此两个额度窗口不会互相占用点数。数据库不包含 OAuth Token、Cookie、Basic Auth 密码或 App API Key。SQLite 可能同时生成 `codex-meter.db-wal` 和 `codex-meter.db-shm`，它们也必须保留在同一个数据目录中。使用 `docker compose down -v` 也不会删除该目录，因为它是宿主机 bind mount。

升级后首次启动时，如果 SQLite 表为空，服务会从现有 JSONL 合并文件、周额度文件、5 小时额度文件、原始归档及其有效 `.bak` 快照迁移可见记录；迁移成功后 SQLite 成为唯一运行时数据源，旧 JSONL 文件保留为安全回退副本。旧版本已经覆盖的更早记录无法凭空恢复，所以迁移后的点数可能暂时少于 48；随着新的周额度值出现，周额度时间线会从完整数据库记录中逐步补齐，且不会再次加入重复值。

SQLite 使用事务、WAL 和忙等待配置，定时采样写入单行记录；数据库初始化和迁移失败时服务不会删除旧 JSONL 文件。详细表结构、迁移和代码目录说明见[存储与工程结构文档](storage.md)。
