...
文件中的每行是一条去重后的采样记录；连续相同的额度值只保留该区间的第一条记录。除兼容用的合并文件外，服务还维护 `data/usage-history-weekly.jsonl` 和 `data/usage-history-five-hour.jsonl` 两条独立历史，各自最多保留最新 48 个变化点，因此五小时额度变化不会挤掉周额度历史，反之亦然。记录包含采样时间、5 小时已使用百分比和本周已使用百分比，不包含 OAuth Token、Cookie、Basic Auth 密码或 App API Key。使用 `docker compose down -v` 也不会删除该目录，因为它是宿主机 bind mount。
