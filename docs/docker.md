...
```text
data/usage-history-raw.jsonl
data/usage-history.jsonl
```

`usage-history-raw.jsonl` 是完整的定时采样归档，每行保留一条原始采样，不按 48 条截断；`usage-history.jsonl` 是兼容用的合并快照。服务还维护 `data/usage-history-weekly.jsonl` 和 `data/usage-history-five-hour.jsonl` 两条展示快照：周额度先从完整原始归档按自身 `used_percent` 去重，再取最近最多 48 个不同百分比；5 小时额度按自身 `five_hour_used_percent` 执行同样规则。相同百分比优先保留最近的正常采样，因此两个额度窗口不会互相占用点数。记录包含采样时间、5 小时已使用百分比和本周已使用百分比，不包含 OAuth Token、Cookie、Basic Auth 密码或 App API Key。使用 `docker compose down -v` 也不会删除该目录，因为它是宿主机 bind mount。

升级后首次启动时，如果不存在原始归档，服务会从现有合并文件、周额度文件和 5 小时额度文件及其有效 `.bak` 快照迁移可见记录，然后从此保留全部新的定时采样。旧版本已经覆盖的更早记录无法凭空恢复，所以迁移后的点数可能暂时少于 48；随着新的周额度值出现，周额度时间线会从完整归档中逐步补齐，且不会再次加入重复值。原始归档会持续增长，请按实际保留周期自行管理数据目录容量。

历史文件采用临时文件完整写入后原子替换，并在各自的 `.bak` 文件中保留上一份有效快照。若主文件为空或包含损坏记录，服务启动时会自动回退读取备份，避免容器重建或进程中断发生在写入期间时丢失整段历史。

额度历史由后端独立定时采集，后台每 5 分钟按时钟整点采集一次，例如 `00、05、10、15` 分钟，并将记录时间标记为对应的 5 分钟时点；服务启动后不会立即补采集。前端手动刷新和自动刷新不会额外写入采样记录。手动刷新仍可请求最新上游数据，但历史记录只由后台采集任务写入。上游异常时，后台可沿用最近一次成功采集的值并标记 `stale: true`；去重时相同百分比优先保留最近的正常采样，只有不存在正常采样时才保留最近的降级采样，避免用失败时点伪装额度变化。即使前端在 00:00–08:00 暂停自动刷新，后端采集任务仍会继续运行。
