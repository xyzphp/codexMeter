...
额度历史由后端独立定时采集，服务启动后立即尝试采集，之后每 5 分钟采集一次；前端手动刷新和自动刷新不会额外写入采样记录。手动刷新仍可请求最新上游数据，但历史记录只由后台采集任务写入。上游异常时，后台会按最近一次成功采集的值追加当前时间的降级记录，并标记 `stale: true`；如果还没有成功采样，则跳过本次记录。即使前端在 00:00–08:00 暂停自动刷新，后端采集任务仍会继续运行。
