...
`weekly_history` 和 `five_hour_history` 是从后端完整定时采样归档计算出的两条独立额度时间线：周额度先按自身 `used_percent` 去重，5 小时额度先按自身 `five_hour_used_percent` 去重；每个相同百分比优先保留时间最近的正常采样，没有正常采样时才保留最近的降级采样，完成去重后再截取最近最多 48 个不同百分比。因此 5 小时额度的变化不会占用周额度的 48 点窗口，反之亦然。旧版 `history` 字段仍返回两条时间线的合并结果，最多可能包含 96 条记录；新客户端应优先使用两个独立字段。
