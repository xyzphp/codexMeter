...
`weekly_history` 和 `five_hour_history` 是两条独立的额度变化时间线：各自按时间升序去除连续重复采样，并最多保留最新 48 个变化点，因此 5 小时额度的变化不会占用周额度的 48 点窗口，反之亦然。旧版 `history` 字段仍返回两条时间线的合并结果，最多可能包含 96 条记录；新客户端应优先使用两个独立字段。
