...
`weekly_history` 和 `five_hour_history` 是两条独立的额度采样时间线：各自按时间升序去除同一时间、同一对应额度值的重复点，并最多保留最新 48 个采样点，因此 5 小时额度的变化不会占用周额度的 48 点窗口，反之亦然。不同时间的相同百分比仍属于不同采样点。旧版 `history` 字段仍返回两条时间线的合并结果，最多可能包含 96 条记录；新客户端应优先使用两个独立字段。
