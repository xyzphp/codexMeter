...
`weekly_history` 和 `five_hour_history` 是两条独立的额度采样时间线：各自按时间升序最多保留最新 48 次采样，即使连续采样的百分比相同也会保留，因此 5 小时额度的采样不会占用周额度的 48 点窗口，反之亦然。旧版 `history` 字段仍返回两条时间线的合并结果，最多可能包含 96 条记录；新客户端应优先使用两个独立字段。
