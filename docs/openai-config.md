...
配置页面按四个标签操作：先填写代理协议、IP/主机和端口并点击“测试并保存，继续配置”；如果不使用代理，点击“使用直连并保存”即可直接保存直连设置，再配置账号凭证。端口默认是 `7890`，页面会自动把三项组合成后端使用的代理 URL。连接测试通过后才会解锁账号凭证页；测试失败不会保存代理配置。页面访问 Tab 可以独立配置管理页面的 Basic Auth，提示音 Tab 用于直接试听额度提醒音；这两个标签都不受代理步骤限制。
