...
配置页面按四个标签操作：在“网络代理”页关闭“使用代理”开关即可使用直连并自动保存；开启开关后填写代理协议、IP/主机和端口，再点击“测试并保存，继续配置”。代理 IP 默认是 `127.0.0.1`，端口默认是 `7890`，页面会自动把三项组合成后端使用的代理 URL。连接测试通过后才会解锁账号凭证页；测试失败不会保存代理配置。页面访问 Tab 可以独立配置管理页面的 Basic Auth，提示音 Tab 用于直接试听额度提醒音；这两个标签都不受代理步骤限制。
