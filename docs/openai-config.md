...
配置页面按三步操作：先填写代理地址并点击“测试并保存，继续配置”（不使用代理时确认直连），再配置账号凭证，最后保存其他运行选项。代理测试通过后才会解锁账号凭证区域；测试失败不会保存代理配置。

账号凭证区域提供“快速配置”向导：把浏览器 Network 中成功的 `/backend-api/wham/usage` 请求复制为 Fetch 或 cURL，粘贴后点击“自动解析并填充”。页面会在本机浏览器中解析 `Authorization`、Cookie、Account ID 和客户端上下文；点击“测试账号连接”会用临时配置请求一次额度接口，测试成功后再点击“保存配置”。测试阶段不会写入 `config.json`，页面也不会回显完整 Token 或 Cookie。
...
在设置页面按以下顺序操作：测试并保存代理（或确认直连）→ 粘贴成功请求并自动解析 → 测试账号连接 → 保存配置。保存接口只返回脱敏后的 `token_hint` 和 Cookie 配置状态，不会回显完整 Token 或 Cookie。若只填写 Token 而保留已有 Cookie，Cookie 输入框留空即可。
