...
配置页面按三个标签操作：先填写代理协议、IP/主机和端口并点击“测试并保存，继续配置”；如果不使用代理，点击“使用直连并保存”即可直接保存直连设置，再配置账号凭证。端口默认是 `7890`，页面会自动把三项组合成后端使用的代理 URL。连接测试通过后才会解锁账号凭证页；测试失败不会保存代理配置。页面访问 Tab 可以独立配置管理页面的 Basic Auth，不受代理步骤限制。
...
设置页面的代理表单分为三项：协议下拉框（HTTP、HTTPS、SOCKS5）、IP/主机和端口。端口默认 `7890`；保存时会生成类似下面的代理 URL：
...
在设置页面按以下顺序操作：测试并保存代理（或确认直连）→ 粘贴成功请求 → 识别并覆盖保存。保存接口只返回脱敏后的 `token_hint` 和 Cookie 配置状态，不会回显完整 Token 或 Cookie。
